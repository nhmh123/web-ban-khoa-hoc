...
@section('admin.content')
    <div class="container-fluid py-4 position-relative">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="update-course-form" action="{{ route('lectures.update', $lecture) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="sec_id" value="{{ $lecture->section->sec_id }}">
                    <!-- Tên bài giảng -->
                    <div class="mb-3">
                    ...
                    <div class="mb-3 form-check">
                        ...
                        <label class="form-check-label" for="is_intro"
                            {{ old('is_intro', $lecture->is_intro) ? 'checked' : '' }}>Là bài giảng
                            giới thiệu?</label>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="type" id="type_video" value="video"
                                {{ old('type', $lecture->type) == 'video' ? 'checked' : '' }} checked>
                    ...
                    <div id="video_form">
                        ...
                        <div class="ratio ratio-16x9" id="video-preview-wrapper">
                            <video id="video-preview" src="{{ $signedUrl }}" controls></video>
                        </div>
                    </div>
            ...
            {{ old('content', $lecture->article->content ?? '') ?? ($lecture->article->content ?? '') }}
        </textarea>
                        </div>
                    </div>
                    ...
                    <ul class="list-group mb-3">
                        @foreach ($attachments as $attachment)
                            <li class="list-group-item d-flex justify-content-between">
                                ...
                                <a href="{{ route('admin.attachments.destroy', $attachment) }}" class="d-block">
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                $('#update-course-form').on('submit', function() {
                    $('#form-overlay').fadeIn();
