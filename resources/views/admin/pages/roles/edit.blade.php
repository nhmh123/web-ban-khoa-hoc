        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('roles.update', $role) }}" enctype="multipart/form-data">
                                    ...
                                    @foreach ($permissionModel as $permission)
                                        <div class="col-md-3">
                                            <input type="checkbox" class="permission form-check-input"
                                                ...
                                                id="{{ $permission->slug }}"
                                                @if (in_array($permission->id, old('permission_id', []))) checked
                                                @elseif (in_array($permission->id, $role->permissions->pluck('id')->toArray())) checked @endif>
                    ...
                    @endforeach
                    <input type="submit" name="btn-add" class="btn btn-primary" value="Cập nhật mới">
                </form>
            </div>
        </div>
    </div>
